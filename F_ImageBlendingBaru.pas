...
var
  ...
  begin
    w:=FormHasil.Image1.Picture.Width;
    h:=FormHasil.Image1.Picture.Height;
    w1:=w;
    h1:=h;
    ...
    for y := 0 to h - 1 do
      begin
        PC:=FormHasil.Image1.Picture.Bitmap.ScanLine[y];
        for x := 0 to w - 1 do
          begin
            Bi[x,y]:=PC[3*x];
            Gi[x,y]:=PC[3*x+1];
...
F_Rotasi.Top:=FormHasil.Top;
F_Rotasi.Left:=FormHasil.Left+FormHasil.Width;
